...
final class OverviewSpeakIntentTests: XCTestCase {
    ...
    func testInboxListFamilySpeaksAckNotRecitation() {
        for ask in Self.inboxListFamily {
            ...
            XCTAssertFalse(InboxGlance.isShortSpokenSummary(replay.reply), ask)
            ...
            XCTAssertFalse(replay.reply.contains("Greenacre"), ask)
            XCTAssertFalse(replay.reply.contains("Closing"), ask)
    ...
    func testCalendarListFamilySpeaksAckNotEventRecitation() {
        for ask in Self.calendarListFamily {
            ...
            XCTAssertTrue(replay.reply.isEmpty, "\(ask) list/show: \(replay.reply)")
            XCTAssertNotEqual(replay.reply, "Here they are.", ask)
            XCTAssertFalse(replay.reply.contains("Massimo"), ask)
