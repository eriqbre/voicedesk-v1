...
Source of truth: [PRODUCT_REQUIREMENTS.md](./PRODUCT_REQUIREMENTS.md). Rename later (Voxa is taken).
...
**Google OAuth + sync** (PRD build slice 3): Sign in with Google, Gmail / Calendar / Tasks read, confirm-gated writes, offline cache for last-synced reads. Then graph links (email ↔ listing ↔ people).
...
VoiceDesk/
  VoiceDeskApp.swift
  AppModel.swift        Conversation, tour, confirm, activity
  Models.swift
  SampleData.swift
  Theme.swift
  Voice/                Grok contracts + mock loop + Google stub
  Views/                Spine + cards
PRODUCT_REQUIREMENTS.md Unchanged product spec
```
